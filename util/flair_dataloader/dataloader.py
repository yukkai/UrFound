...
from util.flair_dataloader.dataset import Dataset, UniformDataset
...
def get_data_list(dataframes_path, data_root_path, datasets, balance=False, batch_size=8, num_workers=0,
               banned_categories=None, caption="A fundus photograph of [CLS]", augment_description=True):
    ...
    for iDataset in datasets:
        ...
        dataframe = pd.read_csv(dataframes_path + iDataset + ".csv")
        print("Processing data: " + iDataset, len(dataframe))

        for i in range(len(dataframe)):
            data_i = dataframe.loc[i, :].to_dict()
            data_i["categories"] = ast.literal_eval(data_i["categories"])
            data_i["atributes"] = ast.literal_eval(data_i["atributes"])
